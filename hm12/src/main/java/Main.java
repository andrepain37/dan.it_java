...
public class Main {
    ...
    private static void writeToFile() {
        FileHandleFamily fhf = new FileHandleFamily();
        fhf.writeToFile(familyController.getAllFamilies());
    }

    private static void loadData() {
        FileHandleFamily fhf = new FileHandleFamily();
        fhf.readFamilyList().stream()
                .forEach(f -> {
    ...
    private static void printCommands(){
        ...
        String commands = "- 1. Сохранить семьи в файл (hm12/familyList.txt)\n" +
                "- 2. Загрузить данные с файла\n" +
                "- 3. Отобразить весь список семей (отображает список всех семей с индексацией, начинающейся с 1)\n" +
                ...
                "- 10. Удалить всех детей старше возраста (во всех семьях удаляются дети старше указанного возраста - будем считать, что они выросли)\n";
